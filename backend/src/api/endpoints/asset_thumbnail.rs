...
pub async fn handler(
    ...
    let file_path = storage.asset_thumbnail_path(project_id, asset_id);
    match fs::read(&file_path).await {
        ...
        Err(e) => {
            tracing::error!("Failed to read thumbnail {:?}: {}", file_path, e);
            HttpResponse::InternalServerError()
                .content_type("text/plain; charset=utf-8")
                .body(format!("Failed to read thumbnail: {e}"))
        }
    }
}
